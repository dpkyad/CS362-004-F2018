...
int main() {
	...
	//set current hand
	testG.hand[thisPlayer][testG.handCount[thisPlayer]-1] = great_hall;
	...
	if(cardsFound == 1 && testG.handCount[thisPlayer] ==  handSize && testG.numActions == currAction+1 && testG.handCount[thisPlayer+1] == 0)
	{
		printf ("PASS: ");
		pass++;
	}
